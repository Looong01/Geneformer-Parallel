...
def get_embs(
    ...
    if not isinstance(model, torch.nn.DataParallel) and torch.cuda.device_count() > 1:
        ...
        main_device = f"cuda:{device_ids[0]}"  # 主设备是第一个GPU
        model_input = model.module  # 访问原始模型
    else:
        main_device = "cuda" if torch.cuda.is_available() else "cpu"
        model_input = model

    
    model_input_size = pu.get_model_input_size(model_input)
    ...
    for i in trange(0, total_batch_length, forward_batch_size, leave=(not silent)):
        ...
        attention_mask = pu.gen_attention_mask(minibatch).to(main_device)

        max_len = int(max(minibatch["length"]))
        original_lens = torch.tensor(minibatch["length"], device=main_device)
        minibatch.set_format(type="torch")

        input_data_minibatch = minibatch["input_ids"]
        input_data_minibatch = pu.pad_tensor_list(
            input_data_minibatch, max_len, pad_token_id, model_input_size
        ).to(main_device)  # 确保数据在主设备
