...
class Classifier:
    ...
    def evaluate_saved_model(
        ...
        **Parameters**
        ...
        if torch.cuda.device_count() > 1:
            ...
            model = torch.nn.DataParallel(model)  # 包装为数据并行模型
    ...
    # ):
    #     """
    #     Evaluate the fine-tuned model with DDP.

    #     ​**Parameters**
    #     model_directory : Path
    #         | Path to directory containing model
    #     id_class_dict_file : Path
    #         | Path to _id_class_dict.pkl (format: numerical IDs: class_labels)
    #     test_data_file : Path
    #         | Path to directory containing test .dataset
    #     output_directory : Path
    ...
    #     # Initialize DDP evaluation
    #     world_size = torch.cuda.device_count()
    #     mp.spawn(
    #         self.evaluate_model_DDP,  # 直接调用成员方法
    #         args=(
    #             world_size,
    #             model_directory,
    #             num_classes,
    ...
    #     # 只在主进程加载结果并生成报告
    #     if dist.get_rank() == 0:
    #         test_metrics_output_path = Path(output_directory) / f"{output_prefix}_test_metrics_dict.pkl"
    #         with open(test_metrics_output_path, "rb") as f:
    ...
    #         if num_classes == 2:
    #             mean_fpr = np.linspace(0, 1, 100)
    #             mean_tpr = all_metrics["roc_metrics"]["interp_tpr"]
    #             all_roc_auc = all_metrics["roc_metrics"]["auc"]
    #             all_metrics["all_roc_metrics"] = {
    #                 "mean_tpr": mean_tpr,
    #                 "mean_fpr": mean_fpr,
    #                 "all_roc_auc": all_roc_auc,
    #             }
    ...
    #     self,
    #     rank,  # mp.spawn自动注入的进程rank
    #     world_size,   # 第二个参数：总进程数 (int)
    #     model_directory,
    #     num_classes,
    #     id_class_dict,
    #     eval_data,
    ...
    #     output_prefix,
    # ):
    #     assert isinstance(world_size, int), "world_size must be integer!"
    #     """DDP评估核心逻辑"""
    #     # 初始化分布式环境
    ...
    #     ##### Evaluate the model #####
    #     labels = id_class_dict.keys()
    #     y_pred, y_true, logits_list = eu.classifier_predict_DDP(
    #         model, self.classifier, eval_data, self.forward_batch_size, rank
    #     )
    #     # 同步所有进程
    #     dist.barrier()
    #     conf_mat, macro_f1, acc, roc_metrics = eu.get_metrics(
    ...
    #             "roc_metrics": roc_metrics,
    #         }
    #         metrics_path = Path(output_directory) / f"{output_prefix}_test_metrics_dict.pkl"
